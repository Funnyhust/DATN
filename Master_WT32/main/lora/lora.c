...
#define CONFIG_CS_GPIO      15   // Chip Select
#define CONFIG_RST_GPIO     2// Reset
#define CONFIG_MISO_GPIO    13 // SPI MISO
#define CONFIG_MOSI_GPIO    23  // SPI MOSI
#define CONFIG_SCK_GPIO     14  // SPI SCK
#define CONFIG_DIO0_GPIO    4 // DIO0 interrupt
...
{
   if (level < 2) level = 2;
   else if (level > 20) level = 20; // Cho phép lên đến 20 dBm

   if (level >= 18) {
      // Chế độ High Power (18-20 dBm)
      lora_write_reg(REG_PA_DAC, 0x87); // Bật PA_DAC
      int output_power = 15 - (20 - level); // Điều chỉnh OutputPower
      lora_write_reg(REG_PA_CONFIG, PA_BOOST | output_power);
   } else {
      // Chế độ bình thường (2-17 dBm)
      lora_write_reg(REG_PA_DAC, 0x84); // Tắt PA_DAC (mặc định)
